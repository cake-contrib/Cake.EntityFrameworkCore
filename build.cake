...
Task("Restore")
    ...
    .Does(() =>
{
    DotNetRestore("./Cake.EntityFrameworkCore.sln", new DotNetRestoreSettings
    {
        LockedMode = true,
    });
});
...
{
    DotNetBuild("./Cake.EntityFrameworkCore.sln", new DotNetBuildSettings
    {
...
Task("Pack")
    ...
    .Does(() =>
{
    DotNetPack("./src/Cake.EntityFrameworkCore/Cake.EntityFrameworkCore.csproj", new DotNetPackSettings
    {
        ...
        {
            Version = buildVersion.Version,
            PackageReleaseNotes = $"https://github.com/cake-contrib/Cake.EntityFrameworkCore/releases/tag/v{buildVersion.Version}"
        }
    });
});
...
Task("Publish")
    ...
    .WithCriteria(() => string.Equals("refs/heads/main", GitHubActions.Environment.Workflow.Ref, StringComparison.OrdinalIgnoreCase) || GitHubActions.Environment.Workflow.Ref.StartsWith("refs/tags/", StringComparison.OrdinalIgnoreCase))
    .Does(async () =>
        await GitHubActions.Commands.UploadArtifact(Directory("./artifact/nuget"), $"Cake.EntityFrameworkCore.{buildVersion.Version}"));
